...
### Context Windows
The context window is the total amount of text (tokens) an LLM can hold in its working memory at a given moment. While frontier models now boast million-token windows (see the [Model Landscape appendix](/handbook/appendix-model-landscape/) for current examples), treating the context window as a bottomless garbage can is a critical mistake.
