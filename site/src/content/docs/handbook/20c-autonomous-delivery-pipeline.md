---
title: "Chapter 20c: The Autonomous Delivery Pipeline"
---
...
[Chapter 20b](/handbook/20b-autonomy-levels/) delegated *one bounded task*: you hand the agent a crisp work order, it runs the inner loop, you review the PR. This chapter chains the whole delivery line — **vague user story → RFC → iOS tasks in Jira → self-iterating code → PR → code review** — so that a single story, dropped in one end, comes out the other as a reviewed pull request.

The moment that chain runs without a human kicking off each step, you have left Level 3. Self-triggering from a work source is the definition of **Level 4 — Orchestrated** ([Chapter 20b §2](/handbook/20b-autonomy-levels/#2-the-five-rungs)). Mechanically, the pipeline is a [prompt chain (Chapter 8)](/handbook/08-prompt-chaining/) that reaches across tools — Jira and GitHub over [MCP (Chapter 21)](/handbook/21-tool-calling-mcp/). But the engineering that makes it *safe* is almost entirely at the front.
...
An agent doesn't get tired or cheaper on the retry — it costs the same tokens to build the wrong thing as the right thing, and then *again* to build the right one. A wrong assumption is never a 1× waste; it's **wrong work plus redo**, compounding the further right it's caught (see [Chapter 35 on agentic cost](/handbook/35-agentic-security-and-cost/)). That single fact justifies the whole design: **spend a human's two minutes at the front to avoid an agent's dollars at the back.**
...
| :-- | :-- | :-- | :-- | :-- |
| S1 | **Story → contract** | Grounds, interrogates, writes an RFC with testable acceptance criteria | Terminal agent + [`rfc-template.md`](https://github.com/gurpreet-ios/ios-ai-playbook/blob/main/templates/rfc-template.md) | **🔒 GATE A — approve the spec** |
| S2 | **Contract → work orders** | Decomposes into layered, bounded iOS subtasks, tests-first | Jira MCP | (folds into Gate A) |
| S3 | **Work order → code** | The local Level-3 loop: plan→edit→`xcodebuild test`→fix→green | Terminal agent + [`AGENTS.md`](https://github.com/gurpreet-ios/ios-ai-playbook/blob/main/AGENTS.md) | exception only |
| S4 | **Raise PR** | Opens PR, auto-fills the AI-context + machine-review checklist | GitHub MCP + [`pr-template.md`](https://github.com/gurpreet-ios/ios-ai-playbook/blob/main/templates/pr-template.md) | exception only |
| S5 | **Code review** | Reviews against `AGENTS.md` + ADRs; iterates on comments | [`claude-autonomy.yml`](https://github.com/gurpreet-ios/ios-ai-playbook/blob/main/.github/workflows/claude-autonomy.yml) | **🔒 GATE B — approve the merge** |
...
**3. Draft the RFC** into [`templates/rfc-template.md`](https://github.com/gurpreet-ios/ios-ai-playbook/blob/main/templates/rfc-template.md), whose newly-added **Acceptance Criteria** section is the linchpin — each criterion written Given/When/Then and mapped to the test that proves it:
...
3. **The gate re-arms downstream.** If S3 later hits a genuine unknown the frozen contract doesn't cover, it must **stop and re-open Gate A**, not guess forward. This is already the standing rule in the [Autonomy Contract](https://github.com/gurpreet-ios/ios-ai-playbook/blob/main/AGENTS.md): *the loop won't close → hand back a diagnosis, don't guess.*
...
Hold those two and the pipeline is aggressive but *legible* — a human sets the destination and inspects the landing, which is exactly the posture [Chapter 20b](/handbook/20b-autonomy-levels/) demands.
...
- **macOS runner cost** → S3 self-iteration runs at local Level 3; CI is reserved for the S5 *gate*, not the grind ([Chapter 35](/handbook/35-agentic-security-and-cost/)).
...
The pipeline reaches across Jira and GitHub, which means it lives or dies on the tools you expose to the agent — proceed to **[Chapter 21: Tool Calling & MCP](/handbook/21-tool-calling-mcp/)** for the MCP servers that make S2 and S4 possible. Then revisit **[Chapter 35: Agentic Security & Cost](/handbook/35-agentic-security-and-cost/)**: a self-triggering delivery line is also a self-triggering *spender*, and the cost table in §2 is only cheap if you meter it.
